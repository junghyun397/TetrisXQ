...
class EnvironmentModel(metaclass=ABCMeta):
    ...
    def get_reward(self):
        reward = 0
        reward += -1 * self.tetris_model.current_score * 0.01

        _, height, deep_hole, roof = self.tetris_model.analysis_board(self.tetris_model.board)
        reward += max(0, height - self._prv_height) * 0.02
        reward += max(0, deep_hole - self._prv_deep_hole) * 0.1
        reward += max(0, roof - self._prv_roof) * 0.1
