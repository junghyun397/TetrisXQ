...
class TetrisXQModel:
    ...
    def _build_model(self, settings):
        ...
        hidden_layer_1 = tf.nn.leaky_relu(tf.matmul(self.X, W1) + b1)
        ...
        hidden_layer_2 = tf.nn.leaky_relu(tf.matmul(hidden_layer_1, W2) + b2)
        ...
        hidden_layer_3 = tf.nn.leaky_relu(tf.matmul(hidden_layer_2, W3) + b3)
    ...
    def _build_optimizer(self, settings):
        ...
        # 경사 하강 최적화
        self.optimizer = tf.train.GradientDescentOptimizer(settings['learningRate']).minimize(self.cost)
