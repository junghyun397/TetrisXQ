...
class TetrisAI(EnvironmentModel):
    ...
    def do_action(self):
        ...
        _, self._def_walls, self._def_height, self._def_holes, self._def_deep_holes, self._def_apertures \
            = self._get_scores(self.tetris_model.get_board_data())

        for index in range(len(states)):
            fulls, wall_sum, height, holes, deep_holes, apertures = self._get_scores(states[index])
            wall_sum = max(wall_sum - self._def_walls, 0)
            height = max(height - self._def_height, 0)
            holes = max(holes - self._def_holes, 0)
            deep_holes = max(deep_holes - self._def_deep_holes, 0)
            apertures = max(apertures - self._def_apertures, 0)

            state_score = fulls * WEIGHT_FULL + wall_sum * WEIGHT_WALL + height * WEIGHT_HEIGHT + holes * WEIGHT_HOLE + deep_holes * WEIGHT_DEEP_HOLE + apertures * WEIGHT_APERTURE
            if state_score > max_score:
                max_score = state_score
                chosen_state = index

        self.tetris_model.board = states[chosen_state]
    ...
    def _get_state_list(self):
        ...
        for x in range(self.board_width):
            for rotate in range(Tetromino.get_rotate_count(self.tetris_model.current_shape_code) + 1):
                if self.tetris_model.rotate_block(0, x):
    ...
    def _get_scores(self, state):
        ...
        return fulls, wall_sum, height, holes, deep_holes, apertures
