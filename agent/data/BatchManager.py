...
class BatchManager:
    ...
    def add_data(self, current_state, next_state, action, reward):
        ...
        self._length = max(self._index + 1, self._max_memory)
    ...
    def get_batch(self, sess, target_function):
        ...
        targets = np.zeros((chosen_batch_size, self._states))

        for index in range(chosen_batch_size):

            random_index = random.randrange(0, self._length)

            current_state = self._current_state[random_index]
            next_state = self._next_state[random_index]
